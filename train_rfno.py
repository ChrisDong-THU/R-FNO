...
parser.add_argument('--cfg', type=str, default="rfno-1")
...
def train(args):
    ...
    # 数据加载器
    train_index = [i for i in range(cfg.data.train_index)]
    val_index = [i for i in range(cfg.data.eval_index[0], cfg.data.eval_index[1])]
    Dataset = dataset_dict.get(cfg.data.type).get(cfg.data.embed)
    
    train_dataset = Dataset(index=train_index, seed=cfg.data.seed, num=cfg.data.num, steps=cfg.data.steps)
    mean, std, img_size = train_dataset.mean, train_dataset.std, train_dataset.size
    val_dataset = Dataset(index=val_index, seed=cfg.data.seed, num=cfg.data.num, mean=mean, std=std, steps=cfg.data.steps)
    ...
    # 实例化模型
    net = RFNO(in_channels=in_channels, img_size=img_size, fno=cfg.fno, gru=cfg.gru, sync=cfg.rfno.sync, model_path="test.pth").cuda()
    net.train()
