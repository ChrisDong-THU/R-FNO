...
def train(args):
    ...
    # 实例化模型
    net = RecFieldViT(in_channels=in_channels, img_size=img_size).cuda()
    net.train()
    ...
    if cfg.train.scheduler == 'e':
        scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=cfg.train.e_scheduler_gamma) # l1适用
